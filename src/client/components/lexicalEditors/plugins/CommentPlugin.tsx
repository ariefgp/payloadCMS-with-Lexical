...
import useLayoutEffect from '../shared/hooks/useLayoutEffect';
...
import {
  ...
  Thread,
  useCommentStore,
} from '../shared/commenting';
import useModal from '../shared/hooks/useModal';
import CommentEditorTheme from '../shared/themes/CommentEditorTheme';
import Button from '../shared/ui/Button/Button';
import ContentEditable from '../shared/ui/ContentEditable/ContentEditable';
...
export default function CommentPlugin({
  ...
  const deleteCommentOrThread = useCallback(
    (comment: Comment | Thread, thread?: Thread) => {
      if (comment.type === 'comment') {
        ...
        const markNodeKeys = markNodeMap.get(id);
        let markNodeKey = Array.from(new Set(markNodeKeys));
        if (markNodeKeys !== undefined) {
          // Do async to avoid causing a React infinite loop
          setTimeout(() => {
            editor.update(() => {
              for (const key of markNodeKey) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < activeIDs.length; i++) {
      ...
      let keyVar = Array.from(new Set(keys));
      if (keys !== undefined) {
        for (const key of keyVar) {
  ...
  useEffect(() => {
    ...
    return mergeRegister(
      ...
      editor.registerMutationListener(MarkNode, (mutations) => {
        let mutationsVar = Array.from(new Set(mutations));
        editor.getEditorState().read(() => {
          for (const [key, mutation] of mutationsVar) {
            const node: null | MarkNode = $getNodeByKey(key);
            let ids: NodeKey[] = [];
